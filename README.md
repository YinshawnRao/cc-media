...
- 独立封面审美评估：[cover-aesthetic-review.md](tools/video/cover-aesthetic-review.md)
...
- `sandbox/<slug>/`：内部测试工程与产物，包括完整 MP4、封面和按需生成的配套文案；不纳入发布版权审核，不因预设版权顾虑降级可用 MV。
...
raw render 与最终 MP4 位于项目 renders/，最终文件名为 renders/<slug>.mp4。新项目默认不创建 publishing/；用户明确要求发布文案和标签时，才生成 publishing/xiaohongshu.md，也可在成片后补充。Git 保存源码、文档、配置和轻量证据，不提交 Cookie、下载媒体、模型、普通 WAV/MP4 或缓存。已有项目不自动迁移版本、声线或视觉。
