...
- 封面先评估本期已有素材切片：贴合大标题与主题、清晰度高、画面纯净、构图适合图文合成且审美评估通过时，可以直接采用，无需额外引入 AIGC。没有足够合适的切片时，默认以 AIGC 无字静态图配合开场配音停留展示兜底。保留封面停留和配音节奏，不强制动态化；选择与制作见 [封面选材与制作流程](CONVENTIONS.md#封面选材与制作流程)。
