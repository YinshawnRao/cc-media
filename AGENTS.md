...
- 每期视觉按作品表达独立选择，封面设计感须贴合作品年代的审美，避免把八九十年代作品包装成明显的 2020 年代画风；复用工程时重新判断封面、配色与字体，设计依据见 [字幕与视觉](CONVENTIONS.md#字幕与视觉)。
- 封面定稿前须比较本期素材切片与具体 AIGC 生成/编辑方案的表现潜力，评估主题吸引力、记忆点、图文构图与年代审美；不能以“截图合格”结束选材。AIGC 有具体提升空间或方案优劣难以判断时，实际出图并比较合成后的手机预览；无明确增益时可说明比较依据后直用优质切片，不强制每期生图。没有合格切片时默认 AIGC 兜底。选择依据纳入独立审美评估；保留封面停留和配音节奏，详见 [封面选材与制作流程](CONVENTIONS.md#封面选材与制作流程)。
- 封面标题、歌手名、年份等信息文字默认用 HTML/CSS 与项目离线字体独立排版，不建议交给图像模型生成。采用 AIGC 时，提示词明确要求不生成文字、为后期排版留出空间；具体流程与例外见 [封面选材与制作流程](CONVENTIONS.md#封面选材与制作流程)。
...
- 代理执行本仓库任务时，禁止启动或控制浏览器、播放器应用及其窗口或标签页进行任何检测，包括查源、预览、封面审美评估、调试、QA 与回听；不得使用 `hyperframes preview`、浏览器自动化或系统打开命令绕过此禁令。
...
- 有旁白的音乐盘点、解说与主题叙事，中文、英文及中英混合歌名须在正文首次介绍该曲时自然念一次；不能以信息卡已展示或精简旁白为由省略。按歌名语言判断，日语、韩语等非中英原名可合理跳过。具体边界见 [歌名口播](CONVENTIONS.md#歌名口播)；这是制作与文案约束，不新增验收检测。
