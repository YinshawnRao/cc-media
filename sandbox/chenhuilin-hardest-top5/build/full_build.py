#!/usr/bin/env python3
"""Build master.wav and HyperFrames HTML for the Kelly Chen (陈慧琳) hardest top 5 video."""
import contextlib
import json
import subprocess
import wave
...
BED = 0.15
VOICE_GAIN = 2.0
MGAIN = {"p5_shuiyuan": 0.82}  # 谁愿放手(慢歌)展示段比快歌高~2dB → 压低对齐（实测后回填）
...
run(
    [
        ...
        f"[voice][music]amix=inputs=2:normalize=0:duration=longest,atrim=0:{intro_dur},alimiter=limit=0.95:level=disabled[out]",
        "-map",
        "[out]",
        "-ac",
        "2",
...
for b in blocks:
    ...
    run(
        [
            ...
            f"[voice][music]amix=inputs=2:normalize=0:duration=longest,atrim=0:{seg_dur},alimiter=limit=0.95:level=disabled[out]",
            "-map",
            "[out]",
            "-ac",
            "2",
...
run(["ffmpeg", "-v", "error", "-f", "concat", "-safe", "0", "-i", "seglist.txt", "-ac", "2", "-ar", "48000", "master_raw.wav", "-y"])
raw_total = dur(ROOT / "master_raw.wav")
# 片尾干净淡出（最后 ~2.1s，落在固定 CTA 收尾之后）。
# 注：段内 afade 经 loudnorm 链路有时不生效，这里在 concat 后对 master 直接淡出，确定可靠。
fade_st = q(raw_total - 2.1)
run([
    "ffmpeg", "-v", "error", "-i", "master_raw.wav",
    "-af", f"afade=t=out:st={fade_st}:d=2.1", "-ac", "2", "-ar", "48000", "master.wav", "-y",
])
planned_total = total
...
videos = [video_tag(0, 0, intro_end, "vert_huahua")]
for idx, b in enumerate(blocks, start=1):
    videos.append(video_tag(idx, b["start"], q(b["end"] - b["start"]), b["clip"]))
videos.append(video_tag(len(videos), outro_start, q(total - outro_start), "vert_huahua"))
...
ranking_rows = "".join(
    ...
    for n, name in [("01", "花花宇宙"), ("02", "失忆周末"), ("03", "不如跳舞"), ("04", "大日子"), ("05", "谁愿放手")]
)
...
body += (
    ...
    '<div><div class="eyebrow">声乐难度盘点</div><h1>陈慧琳最难的5首歌</h1>'
    '<p class="sub">她是公认的电音舞曲女王。可越是轻快好唱的快歌，越难——难的不是飙高音，而是在又快又密的节奏里，气息、律动和控制能不能同时稳住。</p></div>'
    '<div class="chips"><span>电音舞曲</span><span>气息律动</span><span>密集咬字</span><span>边唱边跳</span><span>慢歌控制</span></div></section>'
)
body += "\n" + "\n".join(labels)
body += (
    f'\n<section id="outro" class="clip" data-start="{outro_start}" data-duration="{q(total-outro_start)}" data-track-index="2">'
    '<div class="small">最终榜单</div><h2>陈慧琳的难，是把气息、律动和控制，稳稳唱在又快又密的节奏里。</h2>'
    f'<ol>{ranking_rows}</ol><p class="close">这一期按难度从第五到第一收束：谁愿放手、大日子、不如跳舞、失忆周末、花花宇宙。</p></section>'
)
body += (
...
(ROOT / "meta.json").write_text(json.dumps({"id": "main", "name": "chenhuilin-hardest-top5"}, ensure_ascii=False), encoding="utf-8")
