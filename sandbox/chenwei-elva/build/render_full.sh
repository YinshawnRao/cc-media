#!/usr/bin/env bash
# 渲染全片 10 块（-c 单块、--sdr、崩则重试）→ concat → mux master_full.wav。
PROJ=/Users/yinshawnrao/explorer/cc-media/sandbox/chenwei-elva
HF="$PROJ/hf_full"
cd "$HF" || exit 1
BLOCKS="intro meiyouren turan yuji mingtian woaini woxihuan aida woyao outro"
mkdir -p renders; : > renders/concat.txt
for b in $BLOCKS; do
  target=$(python3 -c "import json;print(json.load(open('blocks.json'))['durs']['$b'])")
  ok=0
  for i in 1 2 3 4 5; do
    pkill -9 -f "Google Chrome for Testing" 2>/dev/null; pkill -9 -f hyperframes 2>/dev/null; sleep 1
    rm -f "renders/$b.mp4"
    npx --yes hyperframes@0.6.69 render -c "$b.html" --output "renders/$b.mp4" --sdr -w auto >/dev/null 2>&1
  ...
  if [ "$ok" != "1" ]; then echo "=== $b FAILED auto, -w1 ==="
    for i in 1 2 3; do
      pkill -9 -f "Google Chrome for Testing" 2>/dev/null; sleep 1; rm -f "renders/$b.mp4"
      npx --yes hyperframes@0.6.69 render -c "$b.html" --output "renders/$b.mp4" --sdr -w 1 >/dev/null 2>&1
      d=$(ffprobe -v error -show_entries format=duration -of csv=p=0 "renders/$b.mp4" 2>/dev/null || echo 0)
      ok=$(python3 -c "print(1 if abs(float('${d:-0}' or 0)-$target)<1.5 else 0)" 2>/dev/null || echo 0)
...
echo "=== mux master_full.wav ==="
ffmpeg -v error -i renders/full_video.mp4 -i "$PROJ/master_full.wav" -map 0:v -map 1:a -c:v copy -c:a aac -b:a 192k -shortest "$PROJ/renders/chenwei-elva-full.mp4" -y
echo "=== DONE ==="
ffprobe -v error -show_entries format=duration:stream=codec_name,width,height,channels -of default=noprint_wrappers=1 "$PROJ/renders/chenwei-elva-full.mp4"
echo "OUT: $PROJ/renders/chenwei-elva-full.mp4"
