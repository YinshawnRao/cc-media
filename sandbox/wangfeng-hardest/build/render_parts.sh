#!/usr/bin/env bash
# 渲染 partA/partB（各 <240s 且 ≤4 video → streaming 开启稳定），-w1 + retry 兜随机GPU崩，concat 成 full_raw.mp4。
cd /Users/yinshawnrao/explorer/cc-media/sandbox/wangfeng-hardest
mkdir -p renders
...
render_one partA.html renders/partA.mp4 114 || exit 1
render_one partB.html renders/partB.mp4 152 || exit 1
echo "=== concat ==="
...
echo "=== mux master.wav ==="
ffmpeg -v error -i renders/full_raw.mp4 -i master.wav -map 0:v -map 1:a -c:v copy -c:a aac -b:a 192k -shortest renders/wangfeng-hardest.mp4 -y
echo "FINAL dur=$(ffprobe -v error -show_entries format=duration -of csv=p=0 renders/wangfeng-hardest.mp4)"
