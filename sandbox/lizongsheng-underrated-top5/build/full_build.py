#!/usr/bin/env python3
"""Build master.wav + HyperFrames HTML for 李宗盛最被低估5首 (倒数 5->1).
...
OUTRO_BED_START = 95.0   # yuanxing(远行)2006演唱段作 outro 床（COVER_SRC=yuanxing，不用歌曲淡出尾）
...
ranking_rows = "".join(
    ...
    for n, name in [("01", "和自己赛跑的人"), ("02", "你像个孩子"), ("03", "远行"), ("04", "希望"), ("05", "一个人")])
...
body += (
    ...
    '<div><div class="eyebrow">华语遗珠 · 唱作盘点</div><h1>李宗盛最被低估的5首歌</h1>'
    '<p class="sub">在《凡人歌》和《山丘》之外，这五首藏在专辑深处的歌，把普通人最难开口的心事，写得那么准。</p></div>'
    '<div class="chips"><span>华语教父</span><span>人生叙事</span><span>唱作之神</span><span>专辑遗珠</span><span>越听越懂</span></div></section>')
body += "\n" + "\n".join(labels)
body += (
    f'\n<section id="outro" class="clip" data-start="{outro_start}" data-duration="{q(total-outro_start)}" data-track-index="2">'
    '<div class="small">完整榜单</div><h2>他最厉害的，不是最高的传唱度，而是把最难开口的心事，写得那么准。</h2>'
...
(ROOT / "meta.json").write_text(json.dumps({"id": "main", "name": "lizongsheng-underrated-top5"}, ensure_ascii=False), encoding="utf-8")
