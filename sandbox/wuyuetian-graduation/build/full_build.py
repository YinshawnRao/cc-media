#!/usr/bin/env python3
"""FULL build — 走出考场那一刻 · 请打开五月天（毕业情绪叙事盘点，5首叙事序）。
结构：封面(纯情绪) → 标题 → 5×(转场卡 + 官方MV letterbox章节) → 结尾/CTA。
情绪线：释然→回望→告别→怅惘→出发。色温弧：夏日白光→黄昏→夜色→冷蓝→明亮。
女声 zf_xiaoyi。无开头长钩子（用户反馈：更快进正题）。
渲染后必须 ffmpeg mux master.wav（HyperFrames 会压平动态）。
"""
import subprocess, contextlib, json, shutil
...
# ---------------- 每首歌定义 ----------------
SONGS = [
 dict(key="zhizu", no="01", name="知足", emo="释然", year="2005", album="神的孩子都在跳舞",
      clip="vert_zhizu", show=32.0, ghost="空", acc="#F0C674", mgain=0.80,
      subline="终于可以，不用再倒数了",
      trans=["第一首，不是庆祝","是你终于放下笔之后","才听见自己的心跳"],
      src="官方 MV · 滚石唱片",
 ...
 dict(key="sgj", no="02", name="时光机", emo="回望", year="2003", album="时光机",
      clip="vert_sgj", show=32.0, ghost="忆", acc="#E8964C", mgain=1.02,
      subline="如果真有时光机，你想回到哪一天？",
      trans=["考完以后才发现","最想回去的","不一定是成绩最好的那天"],
      src="官方 MV · 滚石唱片",
 ...
 dict(key="ganbei", no="03", name="干杯", emo="告别", year="2011", album="第二人生",
      clip="vert_ganbei", show=38.0, ghost="杯", acc="#F2B23E", mgain=1.18,
      subline="干杯，敬这三年 · 也敬后来很难再见的人",
      trans=["真正难过的","不是毕业那天","是有些人，以后真的很难再见了"],
      src="官方 MV · 相信音乐",
 ...
 dict(key="hldwm", no="04", name="后来的我们", emo="怅惘", year="2016", album="自传",
      clip="vert_hldwm", show=32.0, ghost="后", acc="#7FB0D6", mgain=1.27,
      subline="后来，我们会变成谁？",
      trans=["考完以后，未来终于来了","可未来好像","也没那么容易回答"],
      src="官方 MV · 相信音乐",
 ...
 dict(key="zyjs", no="05", name="终于结束的起点", emo="出发", year="2008", album="后青春期的诗",
      clip="vert_zyjs", show=42.0, ghost="起", acc="#F5D27A", mgain=1.18,
      subline="愿你前路很远，也很亮",
      trans=["最后一首","不是结束","是终于，可以开始了"],
      src="官方 MV · 相信音乐",
...
# voice durations
for s in SONGS: s["voice"] = adur(A/f"{s['key']}_voice.wav") if (A/f"{s['key']}_voice.wav").exists() else adur(A/f"s{SONGS.index(s)+1}_voice.wav")
# (voice files are named s1_voice..s5_voice)
...
# ---------------- 绝对时间轴 ----------------
t = 0.0
CH1 = round(TITLE_T + TITLE_D - 0.2, 3)   # first chapter start
starts = {}
t = CH1
for s in SONGS:
...
#  AUDIO
# =====================================================================
# seg_open: cover+title, 知足钢琴床（低，轻起）
def build_open():
    dur = CH1
    ...
    run(["ffmpeg","-v","error","-i",A/"zhizu_intro.wav","-filter_complex",
...
def build_chapter_seg(i, s):
    ...
    run(["ffmpeg","-v","error","-i",C/f"{s['clip']}.mp4","-i",A/f"s{i+1}_voice.wav",
         ...
         f"[voice][music]amix=inputs=2:normalize=0:duration=longest,atrim=0:{dur},alimiter=limit=0.95[out]",
         "-map","[out]","-ac","2","-ar","48000",out,"-y"])

def build_outro():
    dur=OUTRO_D; va=0.4
    run(["ffmpeg","-v","error","-i",A/"zhizu_intro.wav","-i",A/"outro.wav","-filter_complex",
         f"[1:a]aresample=48000,aformat=channel_layouts=stereo,adelay={int(va*1000)}|{int(va*1000)},volume=2.0[voice];"
         f"[0:a]aresample=48000,aformat=channel_layouts=stereo,loudnorm=I=-15:TP=-1.5:LRA=11,"
         f"atrim=0:{dur},asetpts=PTS-STARTPTS,volume=0.2,afade=t=in:st=0:d=1.2,afade=t=out:st={dur-2.5}:d=2.5[bed];"
         f"[voice][bed]amix=inputs=2:normalize=0:duration=longest,atrim=0:{dur},alimiter=limit=0.95[out]",
...
cover_html=f'''<div id="cover" class="layer">
  ...
  <div class="crowd">{crowd()}</div>{ticket}
  <div class="cv-text">
    <div class="cv-eyebrow">毕业季 · 五月天单团歌单</div><div class="cv-rule"></div>
    <div class="cv-h1">走出考场<br>那一刻</div><div class="cv-h2">请打开五月天</div>
...
<div id="chrome_{k}" class="clip chrome" data-start="{f(s['start'])}" data-duration="{f(s['anchor']['dur'])}" data-track-index="{40+i}" style="--acc:{s['acc']}">
  <div class="card">
    ...
    <div class="card-credit">五月天 · {s['year']}<br>《{s['album']}》</div>
  </div>
...
# transition cards (z high, shown at chapter start over footage)
trans_html=[]
for i,s in enumerate(SONGS):
    k=s["key"]; tl=s["trans"]
  ...
  <div class="tr-wrap">
    <div class="tr-l" id="tr1_{k}">{tl[0]}</div>
    <div class="tr-l" id="tr2_{k}">{tl[1]}</div>
    <div class="tr-l accent" id="tr3_{k}" style="color:{s['acc']}">{tl[2]}</div>
  </div>
</div>''')
...
outro_html=f'''<div id="outro" class="layer">
  ...
  <div class="ot-wrap">
    ...
    <div class="ot-sub" id="otsub">这 5 首五月天，送给走出考场的你</div>
    <div class="ot-recap">{recap_rows}</div>
  </div>
  <div class="ot-cta" id="otcta">
    <div class="q1">如果只能选一首五月天陪你毕业</div>
...
{''.join(f'tl.fromTo("#orow{i}",{{opacity:0,x:-16}},{{opacity:1,x:0,duration:.45}},{f(OUTRO_T+3.3+i*0.32)});' for i in range(5))}
...
(HF/"meta.json").write_text('{"id":"main","name":"wuyuetian-graduation"}', encoding="utf-8")
