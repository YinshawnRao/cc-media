#!/usr/bin/env python3
"""Build master.wav + HyperFrames HTML for 杨乃文最被低估的5首歌.

Countdown 5 -> 1 (#1 = 不要来找我 climax). 选源现实（见 SOURCES.md）：
- 如今/路痴(画面)/我给的爱(画面)/不要来找我 无可用棚版 MV（字体可视化/叙事+男主/烧词不固定/静态图）
  → DECOUPLED：音频用官方录音室音轨，画面用本人现场救场
  （如今←NOIR黑2025 / 路痴←2024Live / 我给的爱←2001演唱会蓝光 / 不要来找我←2001演唱会红光）。
- 未接来电 = 真官方 MV（暖调本人特写）→ 耦合（同窗 footage+音频）。
...
# intro 床 = 如今（#5，电子氛围，开场铺底）；outro 床 = 路痴（电子，反思收尾）
INTRO_BED_AUD = "p5"
INTRO_BED_SEEK = 4.0
OUTRO_BED_AUD = "p4"
OUTRO_BED_SEEK = 195.0
...
ITEMS = [
    {
        "key": "p5_ruijin", "no": "05", "aud": "p5",
        "name": "《如今》", "plain": "如今", "meta": "越美丽越看不见 · 2019",
        "show": 27.00, "ch_off": 244.00, "mgain": 1.0,
        "tag": "如今回头，才慢慢看懂",
        "note": "后期电子遗珠，疏离里藏着成熟的清醒",
    },
    {
        "key": "p4_luchi", "no": "04", "aud": "p4",
        "name": "《路痴》", "plain": "路痴", "meta": "越美丽越看不见 · 2019",
        "show": 30.00, "ch_off": 212.90, "mgain": 1.0,
        "tag": "在关系里，找不到方向",
        "note": "不靠大副歌，靠氛围和一点点迷失留住你",
    },
    {
        "key": "p3_weijie", "no": "03", "aud": "p3",
        "name": "《未接来电》", "plain": "未接来电", "meta": "ZERO · 2013",
        "show": 33.00, "ch_off": 150.00, "mgain": 1.25,
        "tag": "一通没接起的电话",
        "note": "不爆发，却全是距离、错过和沉默",
    },
    {
        "key": "p2_wogei", "no": "02", "aud": "p2",
        "name": "《我给的爱》", "plain": "我给的爱", "meta": "Silence · 1999",
        "show": 32.67, "ch_off": 167.66, "mgain": 1.18,
        "tag": "给出去的爱，要不回来",
        "note": "张震岳词曲 · 越冷，越痛的暗伤",
    },
    {
        "key": "p1_buyao", "no": "01", "aud": "p1",
        "name": "《不要来找我》", "plain": "不要来找我", "meta": "Silence · 1999",
        "show": 27.08, "ch_off": 89.61, "mgain": 1.18,
        "tag": "别再靠近，冷里有刺",
        "note": "张震岳词曲 · 最不好惹的杨乃文",
    },
]
...
if not HTML_ONLY:
    # ---- intro: footage video-only slice + bed(学琴钢琴) + voice ----
    run([
...
body += (
    ...
    '<div><div class="eyebrow">华语遗珠 · 摇滚女声的冷面</div><h1>杨乃文<br><b>最被低估</b>的5首歌</h1></div>'
    '<div><p class="sub">在《星星堆满天》《推开世界的门》那些代表作之外，她还有一把更冷、更锋利的嗓子。这五首，藏着最被低估的杨乃文。</p>'
    '<div class="chips" style="margin-top:26px"><span>冷峻疏离</span><span>克制有刺</span><span>魔岩底色</span><span>摇滚女声</span><span>越听越深</span></div></div></section>'
)
body += "\n" + "\n".join(labels)
body += (
    f'\n<section id="outro" class="clip" data-start="{outro_start}" data-duration="{q(total-outro_start)}" data-track-index="2">'
    '<div class="small">最终榜单</div><h2>杨乃文从来不是那种用力讨好的女声，她更像一个冷静的旁观者，把疏离和不肯低头唱得克制又锋利。</h2>'
    f'<ol>{ranking_rows}</ol><p class="close">这些被代表作盖过的遗珠，刚好补全了她最冷、也最被低估的另一面。</p></section>'
)
body += (
...
(ROOT / "meta.json").write_text(json.dumps({
    "id": "main", "name": "yangnaiwen-underrated-top5", "sample": SAMPLE,
