#!/usr/bin/env python3
"""Build master.wav and HyperFrames HTML for the Sammi Cheng (郑秀文) hardest top 5 video."""
import contextlib
import json
import subprocess
import wave
...
BED = 0.15
VOICE_GAIN = 2.0
MGAIN = {"p4_buyao": 1.30, "p2_zhongshen": 1.19}   # +2.3 / +1.5 dB (soft ballad MV / crowd-dynamic live)
...
videos = [video_tag(0, 0, intro_end, "vert_moqi")]
for idx, b in enumerate(blocks, start=1):
    videos.append(video_tag(idx, b["start"], q(b["end"] - b["start"]), b["clip"]))
videos.append(video_tag(len(videos), outro_start, q(total - outro_start), "vert_shake"))
...
ranking_rows = "".join(
    ...
    for n, name in [("01", "煞科"), ("02", "终身美丽"), ("03", "值得"), ("04", "不要惊动爱情"), ("05", "默契")]
)
...
.fullLabel h2{margin-top:14px;font-size:80px;line-height:1.07;font-weight:950}
.fullLabel .tag{margin-top:18px;font-size:41px;line-height:1.28;font-weight:850;color:#f1cf9c}
...
#outro h2{margin-top:18px;font-size:70px;line-height:1.14;font-weight:950;max-width:910px}
...
#outro .close{margin-top:32px;font-size:31px;line-height:1.46;color:#d8d0c5;font-weight:650}
...
body += (
    ...
    '<div><div class="eyebrow">声乐难度盘点</div><h1>郑秀文最难的5首歌</h1>'
    '<p class="sub">百变天后的另一面——快歌的爆发、慢歌的克制，还有粤语的咬字分寸，要同时唱得稳。</p></div>'
    '<div class="chips"><span>快歌爆发</span><span>抒情控制</span><span>粤语咬字</span><span>气息支撑</span><span>女王气场</span></div></section>'
)
body += "\n" + "\n".join(labels)
body += (
    f'\n<section id="outro" class="clip" data-start="{outro_start}" data-duration="{q(total-outro_start)}" data-track-index="2">'
    '<div class="small">最终榜单</div><h2>郑秀文的难，是快歌压得住气场，慢歌收得住情绪。</h2>'
    f'<ol>{ranking_rows}</ol><p class="close">这一期按难度从第五到第一收束：默契、不要惊动爱情、值得、终身美丽、煞科。</p></section>'
)
body += (
...
(ROOT / "meta.json").write_text(json.dumps({"id": "main", "name": "zhengxiuwen-hardest-top5"}, ensure_ascii=False), encoding="utf-8")
