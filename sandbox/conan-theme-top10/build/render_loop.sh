#!/usr/bin/env bash
# 单条 footage_track + 重试循环。本机随机 protocolTimeout/GPU 崩(随机帧)→重试;前2次-w2,之后-w1更稳。
# 关键：每次尝试前删旧 full_raw,成功判定要求正确时长(>=MINDUR),否则会被上次的旧成片骗成假成功。
cd /Users/yinshawnrao/explorer/cc-media/sandbox/conan-theme-top10
export HYPERFRAMES_EXTRACT_CACHE_DIR=/Users/yinshawnrao/explorer/cc-media/sandbox/conan-theme-top10/renders/extract_cache
