#!/usr/bin/env python3
"""Build master.wav + HyperFrames HTML for 张碧晨最难的5首歌.

Countdown 5 -> 1 (#1 = 光的方向 climax). 选源现实（见 SOURCES.md）：
- 张碧晨多数代表作是 OST，官方 MV 全是剧集/电影画面（花千骨/三生三世/消失的她），无张碧晨本人；
  《开往早晨的午夜》是她专辑歌、无剧情 MV。为"唱功难度盘点"+真人封面硬约束，4 首用她本人官方 Live、
  《笼》用《消失的她》官方电影 MV（囚笼意象贴题、电影感、干净）。
- 全部 COUPLED（画面+音频同源）：vert clip 从源 mseek 切，music = raw/<aud>_aud.wav 同源 mseek 切 → 口型同步。
  mseek = ch_off - full_start_local（与切 vert clip 的源时间一致）。
- 封面=动态：intro footage 续在 #5(开往)之前同源连续 → 切第一首画面不剪（删静态 cover_hero）。
...
# intro 床 = 开往早晨的午夜（#5，与封面 footage 同源）；outro 床 = 光的方向（#1，向光而行收尾）
INTRO_BED_AUD = "kaiwang"
INTRO_BED_SEEK = 30.0
OUTRO_BED_AUD = "guang"
OUTRO_BED_SEEK = 150.0
...
ITEMS = [
    {
        "key": "p5_kaiwang", "no": "05", "aud": "kaiwang",
        "name": "《开往早晨的午夜》", "plain": "开往早晨的午夜", "meta": "深夜情歌 · 张碧晨",
        "show": 21.4, "ch_off": 220.57, "mgain": 1.0,
        "tag": "从最黑的夜，开往清晨",
        "note": "不是最炸的那首，却最吃控制",
    },
    {
        "key": "p4_liang", "no": "04", "aud": "liang",
        "name": "《凉凉》", "plain": "凉凉", "meta": "《三生三世十里桃花》· 2017",
        "show": 25.0, "ch_off": 150.36, "mgain": 1.0,
        "tag": "凉凉天意，难在那份清冷",
        "note": "和杨宗纬的合唱，一代人的回忆",
    },
    {
        "key": "p3_long", "no": "03", "aud": "longmv",
        "name": "《笼》", "plain": "笼", "meta": "电影《消失的她》· 2023",
        "show": 30.51, "ch_off": 86.76, "mgain": 1.0,
        "tag": "越唱，越收紧的窒息感",
        "note": "悬疑、压抑、撕裂，全靠情绪密度",
    },
    {
        "key": "p2_nianlun", "no": "02", "aud": "nianlun",
        "name": "《年轮》", "plain": "年轮", "meta": "《花千骨》插曲 · 2015",
        "show": 27.91, "ch_off": 25.69, "mgain": 1.0,
        "tag": "把那股痛，一点一点推开",
        "note": "她最重要的 OST 代表作之一",
    },
    {
        "key": "p1_guang", "no": "01", "aud": "guang",
        "name": "《光的方向》", "plain": "光的方向", "meta": "《长歌行》主题曲 · 2021",
        "show": 30.14, "ch_off": 148.64, "mgain": 1.0,
        "tag": "既要爆发，又要干净漂亮",
        "note": "一首真正的大歌，向光而行",
    },
]
...
if not HTML_ONLY:
    run([
        "ffmpeg", "-v", "error", "-ss", "20", "-i", str(C / "vert_p1_guang.mp4"), "-t", str(outro_dur),
        "-an", "-c:v", "libx264", "-preset", "veryfast", "-r", "30", "-g", "30", "-keyint_min", "30",
        str(CS / "outro.mp4"), "-y",
    ])
    run([
...
#cover h1{margin-top:28px;font-family:"Songti SC",serif;font-size:108px;line-height:1.1;font-weight:900;max-width:944px;text-shadow:0 6px 42px rgba(0,0,0,.70)}
...
.chips span{border:1.5px solid rgba(168,194,234,.48);color:#dfeaf6;background:rgba(7,8,13,.56);font-size:27px;font-weight:800;padding:12px 18px;border-radius:8px}
.fullLabel{position:absolute;z-index:5;left:64px;right:64px;bottom:172px;padding:34px 36px 38px;background:linear-gradient(135deg,rgba(7,8,13,.91),rgba(22,28,42,.56));border-left:8px solid #6f8fc4;border-radius:6px;backdrop-filter:blur(2px);box-shadow:0 22px 70px rgba(0,0,0,.36)}
...
.miniLabel{position:absolute;z-index:5;top:92px;left:58px;display:flex;align-items:center;gap:16px;padding:13px 21px;background:rgba(7,8,13,.70);border:1.5px solid rgba(168,194,234,.50);border-radius:8px}
.miniLabel span{font-family:"JetBrains Mono",monospace;font-size:44px;font-weight:900;color:#eef1f6;background:#07080d;border:1px solid rgba(168,194,234,.48);border-radius:6px;padding:2px 8px;line-height:1.1}
...
#outro li span{font-family:"JetBrains Mono",monospace;font-size:32px;font-weight:900;color:#eef1f6;background:#07080d;border:1px solid rgba(168,194,234,.45);border-radius:6px;padding:4px 8px;line-height:1.1;min-width:58px;text-align:center}
...
body += (
    ...
    '<div><div class="eyebrow">实力唱将 · 难度盘点</div><h1>张碧晨<br><b>最难</b>的5首歌</h1></div>'
    '<div><p class="sub">从《年轮》到《光的方向》，她唱过太多让人单曲循环的歌。可有几首，连她自己都未必敢轻易碰。今天，盘一盘张碧晨唱起来最难的五首。</p>'
    '<div class="chips" style="margin-top:26px"><span>大歌气场</span><span>气息控制</span><span>情绪密度</span><span>古风合唱</span><span>高位穿透</span></div></div></section>'
)
body += "\n" + "\n".join(labels)
body += (
    f'\n<section id="outro" class="clip" data-start="{outro_start}" data-duration="{q(total-outro_start)}" data-track-index="2">'
    '<div class="small">最终榜单</div><h2>张碧晨的厉害，从来不只是高音；而是在收与放之间，把最难唱的歌，唱得干净、又动人。</h2>'
    f'<ol>{ranking_rows}</ol><p class="close">这五首，藏着一个被低估的事实——她，是真能把难度唱成高级感的人。</p></section>'
)
body += (
    f'\n<section id="cta" class="clip" data-start="{q(cta_voice-0.15)}" data-duration="{q(total-cta_voice+0.15)}" data-track-index="5">'
    '<div class="v">为你心里的第一名，评论区投票</div><div class="f">点赞 · 收藏 · 关注</div></section>'
)
...
(ROOT / "meta.json").write_text(json.dumps({
    "id": "main", "name": "zhangbichen-hardest-top5", "sample": SAMPLE,
