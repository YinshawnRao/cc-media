...
HF=/Users/yinshawnrao/explorer/cc-media/sandbox/zwtl-duet-pk/hf
OUT=/Users/yinshawnrao/explorer/cc-media/sandbox/zwtl-duet-pk/renders/full_raw.mp4
TARGET=262.0
mkdir -p /Users/yinshawnrao/explorer/cc-media/sandbox/zwtl-duet-pk/renders
cd "$HF"
for n in $(seq 1 14); do
  ...
  sleep 3
  rm -f "$OUT" 2>/dev/null
  npx --yes hyperframes@0.6.47 render "$HF" --output "$OUT" --sdr -w 1 >/tmp/zwtl_render.log 2>&1
  if [ -f "$OUT" ]; then
  ...
  tr '\r' '\n' < /tmp/zwtl_render.log | grep -oE "(Streaming|Capturing) frame [0-9]+/[0-9]+" | tail -1
done
