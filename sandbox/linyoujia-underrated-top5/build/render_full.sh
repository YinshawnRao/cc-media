#!/usr/bin/env bash
# Render from project root. -w1 + retry: this machine's GPU frame-grab crashes randomly
# with multiple workers (see memory hyperframes-render-flaky-gpu). --sdr mandatory.
...
for i in 1 2 3 4 5 6 7 8; do
  ...
  if [ -f "$OUT" ] && [ "$(ffprobe -v error -show_entries format=duration -of csv=p=0 "$OUT" 2>/dev/null | cut -d. -f1)" -ge 175 ] 2>/dev/null; then
