...
Before choosing a cover, compare the strongest available footage with a concrete AIGC generation/editing direction for theme, emotional appeal, memorability, mobile composition, identity and period fidelity. A usable screenshot alone does not settle the choice. AIGC may retain the real subject and improve its surroundings. If it offers a specific improvement or the choice is uncertain, generate a candidate and compare actual composites at the same required copy, format and viewing size; layouts may suit each image, and the screenshot may still win. If no clear gain is expected, direct footage use is valid with concrete comparative reasons in design.md; label ungenerated alternatives as expectations. If no footage qualifies, use AIGC as fallback. Follow the repository [cover workflow](../../../CONVENTIONS.md#封面选材与制作流程) for trial evidence and genuine tool failures. Preserve the opening narration and cover-viewing pace; this does not require motion or shorter holds.

Both paths use separate HTML/CSS text layers with project-local fonts for titles, artist names, years and other cover copy. When generating artwork, ask for no text and space for later typography. Use the same composition for the exported cover and frame 0. AIGC must use a real image model; direct use of existing footage/stills and HTML/CSS text layout are permitted.

Run the separate [cover aesthetic review](../../../tools/video/cover-aesthetic-review.md) on the actual composite, mobile preview and target crop before the full render; confirm the delivered cover and final frame 0 afterward. Moving footage also needs continuous preview of the actual cover interval. Record the comparative selection rationale (or link design.md), candidate previews when tried, presentation mode, concrete observations and reviewer identity in qa/cover-aesthetic-review.md. Missing comparison or a required but unperformed trial needs review before passing; documented tool exceptions retain their stated limits. A static hold during opening narration is valid and is not a reason for rework. Unsuitable footage with no qualified alternative falls back to AIGC still artwork. A clear mismatch with this brief returns only the cover for rework: scope CSS and footage changes to cover nodes, preserve the rest of the content, audio and timing, and refresh affected technical QA after replacing an in-video cover. Mechanical PASS does not establish aesthetic acceptance.
...
Use [hyperframes-cli](../hyperframes-cli/SKILL.md) for lint/render. Do not open a browser for checks. Render raw and final under renders/, post-mux master.wav, then run the applicable project/final gates. Generate publishing copy and run its gate only when the user explicitly requests it, including after video delivery.
