...
只在需要新生成旁白时执行。本次唯一精确指定的已注册编号/名称/别名优先；其次按下述续集规则继承前一期声线；其余情况由代理根据作品主题、情绪与叙事节奏选择。决策池与策略以 [config.json](config.json) 为准，声音名称和画像以 [registry.json](voices/registry.json) 为准。不按歌手性别机械选声；模糊偏好可作为模型选择依据，不做名称相似度猜测。只有无法可靠判断时用 low 触发一次随机兜底。

续集继承仅在本次任务明确指向某个前作（例如「上一期的第二期」「接着前一期做续集」）时生效。代理先核实前一期作品仍在本地，再从其项目的 `voice-selection.json` 读取 `resolved_voice_id`，确认是当前仍可使用的已注册 Qwen 声线后直接沿用，不重新按本期情绪挑选。只因歌手、题材相同或目录名相似，不能认定为续集；关系不明确、前一期已删除、选择记录缺失或声线无法核实时，照常执行默认模型选声，不凭记忆补造前一期的选择。

继承时在本期 brief 或 design 中记录前一期本地路径和声线 ID，通过现有入口为本期生成独立选择记录：`python3 tools/tts/resolve_voice.py --voice <前一期的resolved_voice_id> -o <本期项目>/voice-selection.json`。只继承声线 ID，不复制前一期整份选择文件或旧配置哈希；随后照常运行已选声线 doctor。本次用户明确要求换声时，以本次要求为准。已有本期选择继续整期复用。
