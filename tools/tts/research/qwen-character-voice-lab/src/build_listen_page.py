...
def main() -> int:
    ...
    for voice in manifest["kokoro_voices"]:
        ...
        if path.is_file():
            marker = (
                "Legacy 常用男/女声基线"
                if voice in {"zm_yunxi", "zf_xiaoyi"}
                else "Legacy 可选基线"
            )
            cards_kokoro.append(audio_card(voice, f"Kokoro 历史对照 · {marker}", path))
...
<header><h1>角色化本地 TTS 试听</h1><p class=\"lead\">同一段盘点文案，先听 8 种 Qwen 原创角色声线，再与 Legacy Kokoro 8 音色公平对照。全部试听副本统一到 -18 LUFS / -1.5 dBTP；请戴耳机，重点听角色辨识度、中文自然度、节奏和长期耐听度。</p><p class=\"tip\">建议先只听“固化后盘点样音”，选出 2–4 个候选；再听母带与混合语言，判断音色一致性和作品名可懂度。</p></header>
...
<section><h2>Legacy Kokoro 8 音色基线</h2><p class=\"section-note\">全部由历史 Kokoro 链路生成；zm_yunxi 与 zf_xiaoyi 只代表旧项目常用男/女声基线，不是当前默认。当前新盘点默认声音由中央 registry 决定。</p><div class=\"grid\">{''.join(cards_kokoro)}</div></section>
</main></body></html>"""
