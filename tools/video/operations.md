...
yt-dlp 在来源检测、搜索和实际下载正常时沿用当前版本；`older than 90 days` 仅是版本年龄提醒，不据此判定来源失败。**升级必须由用户主动、明确发起。** 代理不得因版本提醒、下载阻塞、兼容问题或一般制作/排障授权自行升级，也不得以隔离验证为由先安装或试跑新版。遇到明确阻塞时先诊断并报告；只有用户明确要求升级后，才检查安装管理器与依赖变更范围、保留回退条件，并用隔离版本验证搜索、实际下载/合并及所用分段下载，再切换共享环境。

all_cookies.txt 的用户所有权见 [AGENTS](../../AGENTS.md)。代理只用既定只读工具，绝不修权限或安装 candidate。check_yt_cookie.py 检查静态内容不证明服务端会话新鲜。在线取材开工前运行 `check_source_access.py`，按 [来源启动检测](README.md#来源启动检测)处理结果；制作中出现同类故障也暂停并重新分类。

下载失败区分登录态、播放器/签名兼容、码流不可用、执行权限和网络错误。先核对代理授权/沙箱范围；内部权限不足在授权环境恢复后复测，不当作平台故障。未知故障只做有针对性的最小公开请求或同版本候选/客户端诊断。任一平台 Cookie 失效、明确拦截或排除执行权限问题后仍不通，暂停制作并等待用户确认；不能凭另一平台/公开下载成功自动继续。用户明确接受本期降级后才采用相应替代路径，并记录答复。需要 Cookie 时经 yt_dlp_readonly.py，临时工作副本由其在仓库外创建和清理，日志不输出凭据或原始 info JSON。

用途按 [测试与生产边界](../../CONVENTIONS.md#测试产物与生产产物)处理：sandbox 不执行发布版权审核，不因泛化版权顾虑换掉可用 MV；production 才处理授权与发布风险。DNS、403、登录失败、REQUEST_FAILED、找不到 MV 或实际下载到音源封面都不能单独证明版权拦截。只有平台明确返回版权相关限制时才按该原因记录；单视频限制可换候选，平台不可用仍按来源启动检测暂停确认。本地 release_safety 待复核字段不是平台的版权检测结果。

确认兼容问题时优先使用明确版本的隔离环境恢复；涉及 yt-dlp 新版安装或试跑，仍须先满足上述用户主动发起要求。仍需要新凭据或额外权限时请求具体输入，保留已尝试路径与可恢复现场。
