...
用途按 [测试与生产边界](../../CONVENTIONS.md#测试产物与生产产物)处理：sandbox 不执行发布版权审核，不因泛化版权顾虑换掉可用 MV；production 才处理授权与发布风险。DNS、403、登录失败、REQUEST_FAILED、找不到 MV 或实际下载到音源封面都不能单独证明版权拦截。只有平台明确返回版权相关限制时才按该原因记录，并尝试其他实际可用来源；本地 release_safety 待复核字段不是平台的版权检测结果。
