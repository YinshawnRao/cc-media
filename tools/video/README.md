...
1. 先查看本期已有素材切片，评估其与大标题/主题的关系、清晰度、画面纯净度和图文合成后的构图。合适时直接采用片段或其静帧，记录来源、项目内文件、源时间/静帧位置、哈希及选择理由，随后执行第 4–5 步；这条路径不需要调用 AIGC，也不需要检查生图工具或申请例外。没有达到封面标准的片段时，执行第 2–3 步，以 AIGC 无字静态图兜底。
...
3. 将最终采用的生成图和提示词保存在项目 `assets/`（如 `assets/cover-generation-prompt.txt`）；有参考图时保留原图。在 `design.md` 或已有素材记录中简记用途、实际工具、可取得的模型标识及输入/输出关系，不虚构工具或模型名称。工具生成到项目外的图片须复制到项目内再引用。
...
5. 检查实际封面、首帧和手机裁剪下的图文效果，逐字核对文案、缺字和伪文字残留；执行独立 [封面审美评估](cover-aesthetic-review.md)。动态片段还需查看实际封面区间的连续效果；素材直用未通过评估且无合格替代切片时转 AIGC 兜底，不为省略生图降低标准。若替换已入片的封面，保持原音轨与时间轴，重渲受影响内容并刷新最终成片 QA。
...
整片渲染前按 [独立封面审美评估](cover-aesthetic-review.md) 实际查看图文合成后的完整封面、手机预览和目标裁剪，动态方案另查封面区间的连续效果。在项目 `qa/cover-aesthetic-review.md` 记录本期期望、素材选择依据、展示方式、逐项观察、实际媒体路径/哈希、审阅者身份与结论。静态停留配合开场旁白是正常方案，不因此退回或要求动态化。此环节由 agent 视觉审阅完成，独立于机械门禁，不增加默认用户审批。
