...
1. 检查当前环境实际提供的 imagegen 工具及支持的用法，不依赖 Codex 宿主假设。只有确认工具因环境问题不可用时才可跳过，在 `design.md` 或已有素材记录中写明检查结果、实际错误（如有）及原因，继续用现有素材与排版完成封面。
2. 查看参考原图，按本期表达选择生成或编辑用途，实际调用 imagegen，将经检查的结果融入封面；结合排版与手机尺寸预览继续优化。输入错误或生成效果不佳应修正、迭代，不作为工具不可用的依据。
3. 将最终选用的生成图和采用版本的提示词保存在项目 `assets/`（如 `assets/cover-generation-prompt.txt`）；有参考图时保留原图。在 `design.md` 或已有素材记录中简记用途、实际工具、参考图来源及输入/输出文件的对应关系。工具生成到项目外的图片须复制到项目内再引用，避免依赖宿主应用的缓存路径。
