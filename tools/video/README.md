...
新建、重做或优化封面按 [封面选材与制作流程](../../CONVENTIONS.md#封面选材与制作流程) 选择路径：

1. 查看本期较优素材切片，结合裁剪与文字布局，与一个具体 AIGC 生成/编辑方向比较主题吸引力、记忆点、图文构图和身份/年代保真。按规范在 `design.md` 简记双方优势与局限、预期增益及选择理由，不能以“截图合格”结束。无明确 AIGC 增益时可直用切片，记录来源、项目内文件、源时间/静帧位置及哈希，再执行第 4–5 步，无须检查生图工具。AIGC 有具体提升空间或优劣难判断时，执行第 2–3 步制作候选，再合成对比；没有合格切片时默认 AIGC 兜底。
2. 仅在 AIGC 路径检查当前 agent 的生图能力，优先 imagegen，也可使用其他实际调用图像模型的工具。查看参考原图后按本期表达生成或编辑，提示词明确要求无文字、字母、数字、Logo 和水印，并预留文字空间；不要求模型绘制标题、歌手名等最终文案。意外字形或伪文字先编辑清除或重新生成。不得以代码绘图、网页截图或矢量转 PNG 替代生图；输入或效果问题继续修正，环境故障按规范恢复，所有入口确实不可用时如实记录环境例外。
3. 将最终采用的生成图和提示词保存在项目 `assets/`（如 `assets/cover-generation-prompt.txt`）；有参考图时保留原图。用于实际比较的候选、对应提示词及合成预览也留在项目内，即使最终选回截图。在 `design.md` 或已有素材记录中简记用途、实际工具、可取得的模型标识及输入/输出关系，不虚构工具或模型名称。工具生成到项目外的图片须复制到项目内再引用。
4. 两条路径均在 HyperFrames HTML 中叠加独立文字层，用项目离线字体排版标题、歌手名、年份等信息。封面图与视频首帧使用同一套图文合成，首帧文字完整可见。保留开场配音与封面停留的呈现方式；素材直用可保留自然动态或取静帧，AIGC 默认静态展示，不为改变素材路径强制增加运动或缩短停留。用户明确要求生成艺术字等特殊表达时，按规范记录例外。
5. 有 AIGC 比较候选时，在相同必要文案、目标画幅和观看尺寸下查看两者的完整合成与手机裁剪，布局可随图像调整，选择更符合本期表达的方案；未生成候选时复核预期比较及直用理由。检查实际封面、首帧、文案、缺字和伪文字残留，执行独立 [封面审美评估](cover-aesthetic-review.md)。动态片段还需查看实际封面区间的连续效果；素材直用未通过评估且无合格替代切片时转 AIGC 兜底，不为省略生图降低标准。若替换已入片的封面，保持原音轨与时间轴，重渲受影响内容并刷新最终成片 QA。
...
写旁白时落实 [歌名口播](../../CONVENTIONS.md#歌名口播)：把应念的中文、英文歌名自然写入对应歌曲首次介绍的实际 `text`，再同步 manifest 与 TTS request。中央 TTS 不会从歌曲 `title` 自动补名；日语、韩语等非中英原名按规范合理省略。本条属于文案制作步骤，不增加验收检测。
...
整片渲染前按 [独立封面审美评估](cover-aesthetic-review.md) 实际查看图文合成后的完整封面、手机预览和目标裁剪，动态方案另查封面区间的连续效果。在项目 `qa/cover-aesthetic-review.md` 记录本期期望、作品年代与同期视觉依据、截图与 AIGC 的比较及采用理由（可引用 `design.md`）、展示方式、逐项观察、实际媒体路径/哈希、审阅者身份与结论，单独核对年代审美是否匹配。比较缺失或应生成候选却未执行时先补齐，不以成稿可读直接通过。静态停留配合开场旁白是正常方案，不因此退回或要求动态化。此环节由 agent 视觉审阅完成，独立于机械门禁，不增加默认用户审批。
