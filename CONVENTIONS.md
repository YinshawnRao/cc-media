...
- **默认选择策略（硬约定）**：未唯一指定音色时，代理先根据作品主题、整体情绪、叙事角度与节奏，从 `CV001 / CV002 / CV003 / CV004 / CV008 / CV009 / CV010 / CV011 / CV012 / CV013` 十声线标准池（8 女 2 男）决策一款最匹配的声音；只有模型无法可靠判断时才从同一池随机一次。不得按歌手、演员、人物或创作者性别机械决定配音性别。标准池声音均由 Qwen3-TTS Base + 已选定原创参考母带在 Apple Silicon 本地生成。`tools/tts/config.json` 是决策池、随机兜底池与选择策略唯一真源，`tools/tts/voices/registry.json` 是编号、名称、分组、别名和情绪决策画像唯一真源。
- **任务提示词解析优先级**：
...
- **显式声音**：所有已注册编号、名称、别名与实际样音见 `tools/tts/voices/listen.html`。十声线标准候选在页面中标为“标准候选”；其他正式编号仍可供用户精确指定和历史复现，但不参与模型自动决策。任务提示词中的无效显式选择进入作品情绪模型决策；只有模型无法判断时才随机兜底，并在 selection 中记录原因、候选池与最终结果。
