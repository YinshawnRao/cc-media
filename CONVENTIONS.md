...
- **⚠ 长句（~15s+）独立 `pipeline()` 调用会吞掉开头第一小句（2026-07 实战，`xietingfeng-underrated-top5` 排查）**：TOP 盘点每首的整段旁白（"第X名，《歌名》。它收录在……" 一大段，单次 `pipeline(text, voice=..., speed=1.0)` 调用生成，时长 15-19s）**有很高概率把开头第一句吞掉或吞掉一部分**——具体表现为"第X名"完全没发音，或只剩最后一个字（如"名"）。用 Whisper（`whisper-cli --model .../ggml-large-v3.bin --language zh`）转录旁白 wav 逐句核对才发现（听感/RMS 能量图看不出来，因为吞掉的是内容不是音量）。
  ...
  - **修法（已验证，`tools/tts/narrate.py`/各项目 `narrate_segments.py` 通用）**：给每段旁白文本前面**拼接一句无关紧要的垫话**（如"接下来，"），再送进 `pipeline()`；垫话本身要么被完全吞掉（听不出来，总时长几乎不变），要么偶尔漏出声——但漏出来的"接下来，第一名……"依然是通顺人话，不影响听感。**只处理不了极短句**（<3s 的独立短句一般不受影影响，不需要垫话）。系列里所有"每首一整段旁白"的 BLOCKS 都建议加这层垫话保护，尤其 TOP 盘点的"第X名"开场句。
...
> **"第一首出场歌" = 播放顺序上第一个播的那首**：倒数盘点 = 排在最后一名、最先揭晓的那首（如 #5）；叙事序盘点 = 脚本里第一首。**不是"排名第一"那首**，别搞反。
...
- **标题区默认居中靠上**：竖屏封面标题/描述默认放上半屏或中上区域，避免压底像普通字幕卡，也给下半屏留歌手脸/身体辨识度。
