...
- **适用范围**：以下默认约束规则生效后**新启动且包含旁白的自媒体视频**，包括盘点、叙事、自由探索中的实际旁白以及 AI 音色 MV intro。已有 WAV 的历史项目原样复用；已有 voice/model/reference 记录的工程按原配置复现，不因新默认自动换声。本次选择策略迁移前的已知 config/registry 哈希对由中央配置显式兼容，不能泛化接受任意 stale selection。只有用户明确要求“按当前默认重新配音/迁移”时才切换。
- **默认选择策略（硬约定）**：未唯一指定音色时，代理先根据作品主题、整体情绪、叙事角度与节奏，从 `CV012 / CV013 / CV014 / CV015 / CV016 / CV002 / CV003 / CV008 / CV009 / CV017` 十声线标准池（5 男 5 女）决策一款最匹配的声音；只有模型无法可靠判断时才从同一池随机一次。不得按歌手、演员、人物或创作者性别机械决定配音性别。标准池声音均由 Qwen3-TTS Base + 已选定原创参考母带在 Apple Silicon 本地生成。`tools/tts/config.json` 是决策池、随机兜底池与选择策略唯一真源，`tools/tts/voices/registry.json` 是编号、名称、分组、别名和情绪决策画像唯一真源。
- **任务提示词解析优先级**：
  1. `配音：` / `音色：` / `voice=` 结构化字段中唯一精确匹配的角色编号、正式名称或注册别名。
  2. 若无结构化字段，任务提示词中带配音语境的唯一、肯定式精确匹配。
  3. **未指定、未知、模糊描述、多个冲突候选、无法唯一匹配：进入作品情绪模型决策。**代理必须对照十声线的 `decision_profile`，给出一个池内精确 ID/名称、简短理由和 `high`/`medium` 置信度；禁止名称相似度猜测。“男声/女声/可爱一点/二次元声音”等不能唯一定位的描述也不能直接当作唯一指定。
  4. **仅当模型无法可靠决策**（作品信息不足、模型不可用或置信度为 `low`）时，从同一十声线标准池随机一次；不得因为偏好犹豫就跳过情绪判断。
- **启动解析闸门（每个项目只解析一次）**：把用户原始任务提示词与模型决策交给 resolver，落盘项目级 `voice-selection.json`；结果、完整候选池、决策理由与置信度必须同时记录。intro、全部排名或叙事转场、作品 outro、固定 CTA 都复用这一个文件，不得逐段重新判断或重新抽取。
  ```bash
  python3 tools/tts/resolve_voice.py --task-prompt-file brief.txt \
    --model-choice <CV编号或正式名称> \
    --model-reason '<基于作品整体情绪与叙事表达的简短理由>' \
    --model-confidence high \
    -o sandbox/<slug>/voice-selection.json
  # 无法可靠决策时不传 choice，并明确以 low 触发同池随机兜底
  python3 tools/tts/resolve_voice.py --task-prompt-file brief.txt \
    --model-reason '<无法可靠决策的具体原因>' --model-confidence low \
    -o sandbox/<slug>/voice-selection.json
...
- **显式声音**：当前十声线池的名称、别名与三组实际样音见 `tools/tts/voices/listen.html`；页面底部折叠区保留未进入当前池的正式编号与 Kokoro 历史对照。全部已注册编号以 `python3 tools/tts/narrate.py --list-voices` 和 `tools/tts/voices/registry.json` 为真源。其他正式编号仍可供用户精确指定和历史复现，但不参与模型自动决策。任务提示词中的无效显式选择进入作品情绪模型决策；只有模型无法判断时才随机兜底，并在 selection 中记录原因、候选池与最终结果。
- **标准池晋级稳定性门禁（硬约定）**：新增编号或更换参考母带时，必须为该编号登记与参考 WAV 逐字匹配的 `reference_text`，不得沿用不匹配的全局文本；随后运行 `python3 tools/tts/verify_standard_pool.py`，让当前十声线分别经过生产 `narrate.py`、项目 selection、`verify_voice_usage.py`、WAV/响度/长静音/削波与固定离线 ASR。任一生成命中 `max_tokens`、异常拖长、疑似杂音、不可解码或 ASR 基本不可懂时必须 fail closed，不能进入当前池。`--skip-asr` 只允许用于诊断，不算正式 PASS。
...
- **配音**：新项目若无唯一精确指定，先由模型依据作品情绪与叙事表达从十声线标准池决策；只有模型无法可靠判断时才从同一池随机一次。编号、正式名称或注册别名的唯一精确指定仍优先（见配音规范）。
...
配音：默认由模型按作品主题、整体情绪、叙事角度与节奏从十声线标准池决策；无法可靠判断时才同池随机。也可写已注册 CV 编号 / 正式名称 / 别名（唯一精确指定优先）
...
- intro：先从用户原始 prompt 解析一次项目 `voice-selection.json`，再用 `tools/tts/narrate.py <文案> --selection-file ... -o ... --speed 1.12` 生成“如果某歌手唱《歌名》。”类提示；唯一有效指定按指定，否则按作品情绪与叙事表达从十声线标准池决策，只有模型无法可靠判断时才同池随机。构建脚本会校验 sidecar 与项目选择一致，并修剪 TTS 首尾静音。
...
- **TOP 封面必须保留悬念**：只允许展示选题主题和必要的数量信息；是否显示 `TOP N` 遵循下一条的封面语义去重规则。不得列完整歌单、歌曲顺序、具体名次结果或第 1 名。`N→1` 是脚本、timeline、build 配置与 QA 的内部排序约定，**禁止在封面与 intro 画面绘制 `05→01`、`05->01`、`5→1`、`N→1`、“倒数开始”“倒序揭晓”“从第5名开始”或 `05 / 04 / 03 / 02 / TOP` 这类逐名次方向轨**。逐首揭晓时的单个当前名次数字与 `第X名` 口播仍可使用。intro 也遵守同一规则，后续名次只能在各自歌曲转场时首次揭晓。
- **封面极值标题不重复写 TOP**：若封面主标题已经用“最难 / 最燃 / 最被低估 / 最虐心”等“最……”中文极值描述明确表达榜单角度，就不再叠加独立 `TOP`、`TOP N`、`榜单` 或 `排名` 标签；需要数量时可自然写进主标题（如“最难唱的5首歌”），不要再额外放一个 `TOP 5` 角标。这里判断的是明确极值语义，不是机械匹配任意“最”字。**本规则只约束封面**；intro、逐首转场、口播、项目标题、timeline、发布文案和其他位置不因此禁用 `TOP N`。
